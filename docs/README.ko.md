...
> 저장소 루트의 [README.md](../README.md)를 한국어로 옮긴 것이다. 두 문서는 같은 내용을
> 담으므로 한쪽을 고치면 다른 쪽도 함께 고쳐야 한다. `docs/`의 나머지 설계 문서는 영문이고
> [README.md](README.md)가 그 색인이다.
...
**파라미터 지도** — 이쪽도 실시간 엔진을 구독하지 않는다. 시나리오 비교가 축 하나를 따라
곡선을 그린다면, `parameterMap.ts`는 축 *둘*을 동시에 쓸어 17×17 평면을 채우며 칸마다
반응망을 관찰 창 끝까지 돌린다. 축에는 아홉 가지 설정 중 둘을 올릴 수 있다. 저장형 노드의
공급 설정값, 손상 개시 신호, 그리고 *유지* 수준이다. 유지 축이 따로 있는 이유는 일시형
노드에는 공급 설정값이 아예 없기 때문이다. 엔진은 저장형 노드만 공급값 쪽으로 완화시키므로,
Thrombin이나 Factor IXa를 어떤 수준에 두려면 바깥에서 계속 채워 넣는 수밖에 없다. 칸은 네
가지 결과값 중 하나로 칠하는데, 종단 구조 노드가 기준선 아래에 머문 관찰 창의 비율도 그중
하나다. 보충 입력 계획은 고른 노드를 바깥에서 밀어 올린다. 일회 · 지속 · 일정 간격 반복 세
방식이 있고, 계획을 켜면 평면 전체가 그 계획을 반영해 다시 계산되므로 굶주린 구석이 얼마나
움직이는지 통째로 볼 수 있다. 격자 하나가 실행 수백 번이라 한 번에 끝내지 않고 애니메이션
프레임마다 한 줄씩 채운다.
...
노드는 캐스케이드 순서대로 관 안에 좌에서 우로 놓이고, 노드를 잇는 흐름 엣지는 반응
활동도에 따라 굵기와 밝기가 달라진다. 활성 엣지 위로는 표식이 하류를 향해 흐르고, 억제
엣지는 색과 무관하게 알아볼 수 있도록 점선으로 그린다. 노드 구체의 크기와 밝기는 그
노드의 현재 수준을 따라간다. 6MV4 실험 구조는 Factor IXa 자리를 그대로 차지한다.
...
src/
  ...
  hooks/                   애니메이션 클록, 키보드 컨트롤, 디바운스
  simulation/
    types.ts               공유 계약
    entities.ts            엔티티 레지스트리
    reactions.ts           데이터 주도 반응 그래프
    engine.ts              결정론적 고정 스텝 적분기
    scenarioSweep.ts       시나리오 일괄 실행기와 쓸어보기
    parameterMap.ts        두 축 격자 실행기와 보충 입력 계획
...
소스 트리를 훑는 테스트가 진료 관련 어휘와 결과 판정 어휘를 막는다. 주석과 문서 문자열도
검사 대상이다. 그래서 코드 어디에서도 용량, 처치, 판정 같은 말을 쓰지 않고 노드·엣지·수준·
공급값처럼 그래프를 그대로 가리키는 말만 쓴다. 목록은 `src/tests/policy.test.ts`에 있다.
...
Mol* custom shape은 수준과 반응 활동도를 양자화한 지문이 바뀔 때만 다시 만든다. 반응망이
잠잠하면 재생성이 아예 멎는다. 시나리오 비교의 쓸어보기는 한 번에 실행을 스물두 번 돌리기
때문에, 설정이 잠잠해질 때까지 기다렸다가 따라잡는다. 그동안 가벼운 세 실행과 요약 수치는
슬라이더를 따라 즉시 갱신된다.
...
- Mol* 쪽은 반응 활동도를 흐름 엣지의 굵기·밝기와 이동 표식으로 보여 주지만, 개별 반응
  event 하나하나에 붙는 pulse 애니메이션은 아직 legacy 장면에만 있다.
